...
function gotStyles(frames, frametype, the) {
    ...
    export default function UpdateTheAdition({idr,firbas}){
        ...
        const [l_e_cyl,setL_e_cyl]= useState('');
        const [l_e_sph,setL_e_sph]= useState('');

        const [r_e_axis,setR_e_axis]= useState('');
        const [r_e_cyl,setR_e_cyl]= useState('');
        const [r_e_sph,setR_e_sph]= useState('');
        ...
        const handleSubmit = async (e) => {
            ...
            // setRun(prev=>prev+1);
            try {
            await updateDoc(doc(db, `${firbas}`,`${idr.id}`), {
                date,
                daydate,
                monthdate, 
                yeardate,
        ...
        const the =useTheme();
            ...
            const handleChangeframe = (event) => {
                ...
                useEffect(()=>{
                    setName(idr.name);
                    setNumberphone(idr.numberphone);
                    setData(idr.date);
                    setReight(idr.reight);
                    setLeft(idr.left);
                    setIp_d(idr.ip_d);
                    setI_opR(idr.i_opR);
                    setI_opL(idr.i_opL);
                    setR_e_axis(idr.r_e_axis);
                    setR_e_cyl(idr.r_e_cyl);
                    setR_e_sph(idr.r_e_sph);
                    setL_e_axis(idr.l_e_axis);
                    setL_e_cyl(idr.l_e_cyl);
                    setL_e_sph(idr.l_e_sph);
                    setlenstype(idr.lenstype);
                    setLensprice(idr.lensprice);
                    setFrametype(idr.frametype);
                    setFrameprice(idr.frameprice);
                    setTotalprice(idr.totalprice);
                    setPaidprice(idr.paidprice);
                    setRestprice(idr.restprice);
                    setEnterance(idr.enterance);
                    setNote(idr.note);
                    setdayData(idr.daydate);
                    setmonthData(idr.monthdate);
                    setyearData(idr.yeardate);
                },[])
